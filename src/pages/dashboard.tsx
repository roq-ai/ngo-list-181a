...
import { getNgos } from 'apiSdk/ngos';
...
function DashboardPage() {
  ...
  const ngoStat = useSWR<PaginatedInterface<NgoInterface>>(
    () => `/ngos`,
    () => getNgos({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Ngos',
      result: ngoStat?.data?.totalCount,
      isLoading: ngoStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Payments',
