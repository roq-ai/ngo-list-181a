...
import { getNgos, deleteNgoById } from 'apiSdk/ngos';
import { NgoInterface } from 'interfaces/ngo';

type ColumnType = ColumnDef<NgoInterface, unknown>;

interface NgoListPageProps {
...
export function NgoListPage(props: NgoListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<NgoInterface>>(
    () => `/ngos?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: NgoInterface) => {
    if (hasAccess('ngo', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/ngos/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/ngos/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('ngo', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/ngos/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('ngo', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="ngo" />
        </Flex>

        {hasAccess('ngo', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/ngos/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
