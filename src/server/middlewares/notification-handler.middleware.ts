...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  ...
  'expense.submit.create': {
    roles: ['administrator', 'finance-manager'],
    key: 'expenseId',
    tenantPath: [],
    userPath: [],
  },
  'report.generate.create': { roles: ['administrator'], key: 'reportId', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['administrator'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['administrator', 'finance-manager', 'member'];
